...
namespace LightControllerApp.Core
{
    public class GUICamera : IDisposable
    {
        ...
        private readonly double RENDERFPS = 100;
        ...
        public void StartSoftwareTriggerScan()
        {
            ResetGrabStatistics();
            Configuration.AcquireContinuous(camera, null);
        ...
        public void SetSoftwareTrigger(bool on)
        {
            if (camera != null && camera.IsOpen)
            {
                camera.Parameters[PLCamera.TriggerSelector].TrySetValue(PLCamera.TriggerSelector.FrameStart);
                camera.Parameters[PLCamera.TriggerMode].TrySetValue(on ? PLCamera.TriggerMode.On : PLCamera.TriggerMode.Off);
                camera.Parameters[PLCamera.TriggerSource].TrySetValue(PLCamera.TriggerSource.Software);
            }
        }
        ...
        public void StopSoftwareTriggerScan()
        {
            if (camera != null && camera.StreamGrabber.IsGrabbing)
            {
